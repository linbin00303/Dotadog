...
#define UIScreen16_9 CGRectMake(0, 20, [UIScreen mainScreen].bounds.size.width , (([UIScreen mainScreen].bounds.size.width * 9) / 16))
...
+ (instancetype)presentFromViewController:(UIViewController *)viewController withTitle:(NSString *)title URL:(NSURL *)url isLiveVideo:(BOOL)isLiveVideo isOnlineVideo:(BOOL)isOnlineVideo isFullScreen:(BOOL)isFullScreen completion:(void (^)())completion {
    ...
    /** 对播放器控制器进行操作 */
    [viewController presentViewController:PlayerVC animated:YES completion:completion];

    return PlayerVC;
}

#pragma mark - 初始化-push出播放器控制器
+ (instancetype)pushFromViewController:(UINavigationController *)navViewController withTitle:(NSString *)title URL:(NSURL *)url isLiveVideo:(BOOL)isLiveVideo isOnlineVideo:(BOOL)isOnlineVideo isFullScreen:(BOOL)isFullScreen completion:(void (^)())completion {
    IJKHistoryItem *historyItem = [[IJKHistoryItem alloc] init];
...
//    [viewController presentViewController:PlayerVC animated:isFullScreen completion:completion];
    ...
    [navViewController pushViewController:PlayerVC animated:isFullScreen];
...
- (void)viewDidLoad {
    ...
    NSLog(@"---------------------------------%@", [IJKHistory instance].list);
    IJKHistoryItem *item = [IJKHistory instance].list[0];
    self.titleLabel.text =  item.title;
//    self.titleLabel.text = ([IJKHistory instance].list[0]);
...
//    [self.player setScalingMode:IJKMPMovieScalingModeAspectFill];
    ...
    rotationView.backgroundColor = [UIColor grayColor];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (self.isOnlineVideo) {
        ...
        self.onlinePlayView.overlayPanel.frame = self.view.bounds;
    }else{
        [self.player.view mas_makeConstraints:^(MASConstraintMaker *make) {
            NSLog(@"+++++++++++%@",self.view);
            make.top.bottom.right.left.equalTo(self.view);
        }];
        //        NSLog(@"--------%@",self.player.view);
//        self.player.view.frame = self.view.bounds;
        self.playView.overlayPanel.frame = self.view.bounds;
        self.playViewFullScreen.overlayPanel.frame = self.view.bounds;
    }
...
//        self.player.view.frame = self.view.bounds;
    ...
    // 移除归档数据
    [[IJKHistory instance] removeAtIndex:0];
...
//    RAC(self.playView.smallPlayOrPause,selected) = RACObserve(self.player,isPlaying);
